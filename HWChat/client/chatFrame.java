...
//public class chatFrame extends Application implements Runnable
public class chatFrame extends JFrame implements Runnable
{
	//GUI elements
	JPanel inputPanel, outputPanel;
	public JTextField inputMessageTextBox;
	JButton sendMessageButton;
	JMenuBar menuBar;
	JMenu fileMenu, helpMenu, channelsMenu;
	JMenuItem exitMenuItem, autoJoinChannelsMenuItem, aboutMenuItem, friendsMenuItem, reconnectMenuItem;
	JTabbedPane chatTabbedPane;
	chatAboutFrame aboutFrame = new chatAboutFrame();
	friendsList friendsListFrame;
	...
	String serverIP;
	int portNumber;
	chatFrame refToThisChatFrame;
	Point mouseLocation = new Point();
	boolean isAutoAFKOn = false;
	int autoAFKCounter = 0;
	
	
	Timer pingTimer = new Timer(10000, new ActionListener()
	{
		public void actionPerformed(ActionEvent event)
		{
			if (keepCheckingForMessages())
			{
				try
				{
					messageOutputStream.writeObject(new pingChatMessage());
	...
	Timer autoAFKTimer = new Timer(60000, new ActionListener()
	{
		public void actionPerformed(ActionEvent event)
		{
			try
			{
				if (isAutoAFKOn)
				{
					...
					if (p.getX() == mouseLocation.getX() && p.getY() == mouseLocation.getY())
					{
						if ((autoAFKCounter++)==5)
						{
	...
	{
		super("Chat Frame"); //call the constructor for the JFrame class
		setSize(800,400);
		...
		friendsMenuItem = new JMenuItem("Friends/Ignores");
		friendsMenuItem.addActionListener( new ActionListener()
		{
			public void actionPerformed(ActionEvent event)
			{
		...
		reconnectMenuItem.addActionListener( new ActionListener()
		{
			public void actionPerformed(ActionEvent event)
			{
				try
				{
					...
					chatPaneMap = new HashMap<String, chatPane>();
					connectToServer();
					(new Thread(refToThisChatFrame)).start();
					
					reconnectMenuItem.setEnabled(false);
					keepCheckingForMessages = true;
					pingTimer.start();
					inputMessageTextBox.setVisible(true);
				...
				try
				{
					...
					if (!inputText.equals(""))
					{
						...
						if (inputText.substring(0,1).equals("/")) //if the first character is a / the you know it is a command
						{
							...
							else if (splitMessage[0].equalsIgnoreCase("/sc") || splitMessage[0].equalsIgnoreCase("/cc") || splitMessage[0].equalsIgnoreCase("/jc"))
							{
							...
							else if (splitMessage[0].equalsIgnoreCase("/lc"))
							{
								...
								{
									messageOutputStream.writeObject(new commandChatMessage("Don't Set Username Here","No Message",splitMessage[1],commandChatMessage.LC_COMMAND));
								}
								else if (checkChatMessageChannel(channelName)) //if you're not in ~Chat Messages Channel
							...
							{
								String information[] = splitMessage[1].split(" ",2); //split it so that the user name is in the first element and the color is in the second
								if(information.length > 1) {								
							...
							else if (splitMessage[0].equalsIgnoreCase("/ignore"))
							{
								if (splitMessage.length > 1)
								{
									messageOutputStream.writeObject(new commandChatMessage("Don't Set Username Here","No Message",commandChatMessage.IGNORE_COMMAND,splitMessage[1]));
								}
							}
							...
							else if (splitMessage[0].equalsIgnoreCase("/startSpellChecker"))
							{
								messageOutputStream.writeObject(new commandChatMessage("Don't Set Username Here","No Message", commandChatMessage.TOGGLE_SPELL_CHECKING_COMMAND, splitMessage[1]));
							}
							else if (splitMessage[0].equalsIgnoreCase("/system"))
							{
								if (checkChatMessageChannel(channelName)) //if you're not in ~Chat Messages Channel
									messageOutputStream.writeObject(new commandChatMessage("Don't Set Username Here",splitMessage[1],channelName,commandChatMessage.SYSTEM_COMMAND));
							}
							else if (splitMessage[0].equalsIgnoreCase("/getOnlineList"))
							{
								messageOutputStream.writeObject(new commandChatMessage("Don't Set Username Here","no message","no channel name",commandChatMessage.GET_ONLINE_LIST_COMMAND));
							}
							else if (splitMessage[0].equalsIgnoreCase("/autoAFK"))
							{
								isAutoAFKOn = !isAutoAFKOn;
							}
	...
	public void connectToServer()
	{
		try
		{
			...
			String key = "Joe is awesome!";
			messageOutputStream.writeObject(crypt(password.getBytes(),key.hashCode()+""));
			...
			friendsListFrame = new friendsList(messageOutputStream);
  ...
  public String crypt (byte [] data,String key)
  {
			...
			switch (cCM.getCommand())
			{
				...
				case commandChatMessage.LC_COMMAND:
					...
					if (tabIndex != -1)
					{
						...
						if (chatPaneMap.containsKey(cCM.getChannelName()))
							chatPaneMap.remove(cCM.getChannelName());
					}
					break;
			}
	...
	public void disconnect()
	{
		...
		Object channelReferences[] = chatPaneMap.values().toArray();
		
		for (int i = 0; i < channelReferences.length; ++i)
			((chatPane)channelReferences[i]).addMessage(new normalChatMessage("!System!","Your connection to the chat server has dropped. Please restart your client to reconnect.",""));
		
		inputMessageTextBox.setVisible(false);
		sendMessageButton.setVisible(false);
		
		reconnectMenuItem.setEnabled(true);
	}
	
	public synchronized void closeChatConnection()
	{
		try
		{
